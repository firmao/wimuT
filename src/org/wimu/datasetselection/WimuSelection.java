	...
	private static String execQueryRDFRes(String cSparql, String dataset) {
		...
		try {
			...
			if (resultSet != null) {
				...
				Util.updateCount(Approach.WIMU_DUMP, count);
				ret = "WIMU_DUMP: " + dataset + ": " + Util.mAppRes.get(Approach.WIMU_DUMP);
			}
...
//			}
	...
	public static WimuResult execQuery(String cSparql, boolean b, long timeout)
		...
		//for (String ds : mUDataset.values()) {
		for (Entry<String, String> en : mUDataset.entrySet()) {
			String ds = en.getValue();
			String uri = en.getKey();
			ret.setBestDataset(ds);
			size++;
			//if ((ds != null) && ds.contains("dbpedia")) {
			if(uri.toLowerCase().contains("dbpedia")){
				try {
